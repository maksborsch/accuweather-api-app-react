...
import {
  ...
  setSelectedLocation,
} from "../../actions/actionsForecast";

const Forecast = () => {
  const isMetric = useSelector((state) => state.tempType.isMetric);

  const currentConditions = useSelector(
    (state) => state.currentConditions.conditions
  );
  const locationForecast = useSelector(
    (state) => state.locationForecast.forecast
  );
